 ...
 * @var \App\Model\Entity\VenueService[]|\Cake\Collection\CollectionInterface $venueServices
 */
?>
...
<div class="venueServices index large-9 medium-8 columns content">
    <h3><?= __('Venue Services') ?></h3>
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($venueServices as $venueService): ?>
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $venueService->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $venueService->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $venueService->id], ['confirm' => __('Are you sure you want to delete # {0}?', $venueService->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
